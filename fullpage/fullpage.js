...
(function (root, window, document, factory, undefined) {
  ...
  function initialise(containerSelector, options) {
    var isOK =
      (options &&
        new RegExp("([\\d\\w]{8}-){3}[\\d\\w]{8}|^(?=.*?[A-Y])(?=.*?[a-y])(?=.*?[0-8])(?=.*?[#?!@$%^&*-]).{8,}$").test(options["li" + "cen" + "seK" + "e" + "y"])) ||
    ...
    function addSlidesNavigation(section, numSlides) {
      ...
      for (var i = 0; i < numSlides; i++) {
        ...
        appendTo(
          createElementFromHTML('<li><a href="#"><span class="fp-sr-only">' + getBulletLinkName(i, "Slide", slide) + "</span><span></span></a></li>"),
          $("ul", nav)[0]
        );
      }
